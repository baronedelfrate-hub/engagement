...
import { uploadAnaliseFile, deleteAnaliseFile, downloadAnaliseFile } from '@/services/uploadAnaliseService';
...
const UploadBox = ({ title, fieldType, clienteFasesId, currentUrl, currentName, onUploadSuccess, onDeleteSuccess }) => {
  ...
  const handleFileSelect = async (file) => {
    ...
    try {
      const result = await uploadAnaliseFile(file, clienteFasesId, fieldType);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteAnaliseFile(currentUrl, fieldType);
      ...
      {currentUrl ? (
        <div className="bg-background border border-blue-100 dark:border-blue-900 rounded-lg p-4 flex items-center justify-between shadow-sm flex-1">
          ...
          <div className="flex items-center space-x-2 shrink-0">
            <Button size="icon" variant="ghost" onClick={() => downloadAnaliseFile(currentUrl, currentName)} className="text-muted-foreground hover:text-blue-600">
...
export default function AnaliseUploadSection({ clienteFasesId, uploads = {}, onUploadSuccess, onDeleteSuccess }) {
  if (!clienteFasesId) return null;

  return (
    <div className="mb-8">
      <div className="mb-4">
        <h3 className="text-xl font-bold text-foreground">Procedimento Aprovado</h3>
        <p className="text-sm text-muted-foreground">Anexe o Procedimento Aprovado desta fase.</p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <UploadBox 
          title="Procedimento Aprovado" 
          fieldType="procedimento_aprovado"
          clienteFasesId={clienteFasesId}
          currentUrl={uploads.procedimento_aprovado_url}
          currentName={uploads.procedimento_aprovado_nome}
