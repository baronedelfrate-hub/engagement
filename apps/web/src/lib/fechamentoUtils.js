...
export const getStatusColor = (status) => {
  switch (status?.toLowerCase()) {
    case 'em aberto': return 'bg-red-100 text-red-800 border-red-200';
    case 'em andamento': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
    case 'aguardando contabilidade': return 'bg-blue-100 text-blue-800 border-blue-200';
...
export const getStepColor = (numero_etapa) => {
  if (numero_etapa >= 1 && numero_etapa <= 4) return 'bg-blue-50 text-blue-700 border-blue-200'; // Financeiro
  if (numero_etapa >= 5 && numero_etapa <= 7) return 'bg-emerald-50 text-emerald-700 border-emerald-200'; // Fiscal
  if (numero_etapa === 8 || numero_etapa === 9) return 'bg-orange-50 text-orange-700 border-orange-200'; // Administrativo
  if (numero_etapa === 10) return 'bg-purple-50 text-purple-700 border-purple-200'; // Contabilidade
