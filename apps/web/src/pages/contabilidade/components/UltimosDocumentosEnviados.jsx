...
export default function UltimosDocumentosEnviados({ data = [] }) {
  ...
  const getStatusBadge = (status) => {
    ...
    if (s === 'enviado' || s === 'concluído') return <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200">Enviado</Badge>;
              ...
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((row) => (
                  <TableRow key={row.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <TableCell className="text-slate-600">{row.numero || '-'}</TableCell>
                    <TableCell className="text-slate-600">
                      {row.data_envio ? format(new Date(row.data_envio), 'dd/MM/yyyy HH:mm') : '-'}
                    </TableCell>
                    <TableCell className="text-slate-600">{row.destinatario || 'Contabilidade'}</TableCell>
                    ...
                    <TableCell className="text-right flex items-center justify-end gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-orange-600" title="Reenviar">
