  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl font-semibold text-slate-800">Listagem de Lançamentos</h2>
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
        <div className="flex gap-6">
          <div>
            <p className="text-xs text-slate-500 font-semibold uppercase">Total Exibido</p>
            <p className="text-lg font-bold text-slate-800">{formatCurrency(totals.debitos)}</p>
          </div>
          <div>
            <p className="text-xs text-slate-500 font-semibold uppercase">Lançamentos</p>
      ...
      <Card className="border-slate-200 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={8} className="text-center py-12 text-slate-500"><Loader2 className="w-6 h-6 animate-spin mx-auto mb-2" />Carregando...</TableCell></TableRow>
              ) : lancamentos.length === 0 ? (
                <TableRow><TableCell colSpan={8} className="text-center py-12 text-slate-500">Nenhum lançamento encontrado.</TableCell></TableRow>
              ) : (
                lancamentos.map((item) => (
                  <TableRow key={item.id} className="hover:bg-slate-50">
                    ...
                    <TableCell>
                      <div className="font-medium text-slate-800">{formatDate(item.data)}</div>
                      <div className="text-xs text-slate-500">{formatCompetencia(item.competencia)}</div>
                    </TableCell>
                    <TableCell>
                      <div className="max-w-[200px] truncate font-medium text-slate-700" title={item.historico}>{item.historico}</div>
                      <div className="text-xs text-slate-500 flex items-center gap-1 mt-1">
                        {getOrigemIcon(item.origem, "w-3 h-3")} {item.origem} {item.documento_vinculado && `- ${item.documento_vinculado}`}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="text-sm font-medium text-slate-800">{item.conta_debito?.codigo}</div>
                      <div className="text-xs text-slate-500 truncate max-w-[150px]">{item.conta_debito?.nome}</div>
                    </TableCell>
                    <TableCell>
                      <div className="text-sm font-medium text-slate-800">{item.conta_credito?.codigo}</div>
                      <div className="text-xs text-slate-500 truncate max-w-[150px]">{item.conta_credito?.nome}</div>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-blue-600" title="Ver Detalhes" onClick={() => navigate(`/contabilidade/lancamentos/${item.id}`)}>
                          <Eye className="w-4 h-4" />
                        </Button>
                        {canEditLancamento(item.status) && (
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-amber-600" title="Editar" onClick={() => navigate(`/contabilidade/lancamentos/${item.id}/editar`)}>
                            <Edit className="w-4 h-4" />
                          </Button>
                        )}
                        {canDeleteLancamento(item.status) && (
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-red-600" title="Excluir" onClick={() => handleDelete(item.id)}>
