  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-slate-200">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-500">Total Pendências</p>
              <p className="text-2xl font-bold text-slate-800">{totalPendencias}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-slate-200">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-500">Em Aberto/Tratamento</p>
              <p className="text-2xl font-bold text-slate-800">{emAberto}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-slate-200">
        ...
        <Card className="border-slate-200">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-500">Resolvidas</p>
              <p className="text-2xl font-bold text-slate-800">{resolvidas}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-xl font-semibold text-slate-800">Listagem de Pendências</h2>
      ...
      <Card className="border-slate-200 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={8} className="text-center py-12 text-slate-500">Carregando...</TableCell></TableRow>
              ) : filteredPendencias.length === 0 ? (
                <TableRow><TableCell colSpan={8} className="text-center py-12 text-slate-500">Nenhuma pendência encontrada.</TableCell></TableRow>
              ) : (
                filteredPendencias.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id} className={`hover:bg-slate-50 ${overdue ? 'bg-red-50/30' : ''}`}>
                      <TableCell>
                        <div className="font-medium text-slate-800">{item.empresa?.nome_fantasia || item.empresa?.razao_social}</div>
                      ...
                      <TableCell>
                        <div className="font-medium text-slate-800 truncate max-w-[300px]" title={item.descricao}>{item.descricao}</div>
                        <div className="text-xs text-slate-500">Ref: {item.origem}</div>
                      </TableCell>
                      ...
                      <TableCell className="text-center">
                        <span className="text-sm font-bold text-slate-700">{dias}</span>
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-blue-600" title="Ver Detalhes" onClick={() => navigate(`/contabilidade/pendencias/${item.id}`)}>
                            <Eye className="w-4 h-4" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-amber-600" title="Editar" onClick={() => navigate(`/contabilidade/pendencias/${item.id}/editar`)}>
