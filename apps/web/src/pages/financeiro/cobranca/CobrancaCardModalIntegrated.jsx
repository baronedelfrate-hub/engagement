      ...
      <DialogContent className="sm:max-w-[700px] bg-slate-950 border-slate-800 text-slate-100 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            <div className="flex flex-col">
                ...
                <span className="text-xs text-slate-500 font-mono mt-1">Título: {card.numero}</span>
            </div>
            <div className="flex gap-2">
                <Badge variant="outline" className={`${prazoColor} border-0`}>
                    {dias < 0 ? `${Math.abs(dias)} dias atraso` : `${dias} dias prazo`}
                </Badge>
                <Badge className="bg-slate-800 text-slate-300">{card.fase}</Badge>
            </div>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            ...
            <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-800 space-y-2">
                <h4 className="text-xs font-semibold text-slate-400 uppercase flex items-center gap-1"><User className="w-3 h-3" /> Identificação</h4>
                <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                        <span className="text-xs text-slate-500 block">Banco</span>
                        <span className="text-slate-200">{card.banco?.nome || '-'}</span>
                    </div>
                    <div>
                        <span className="text-xs text-slate-500 block">Vencimento</span>
                        <span className="text-slate-200">{new Date(card.data_vencimento).toLocaleDateString()}</span>
                    </div>
            ...
            {/* Classificação */}
            <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-800 space-y-2">
                 <h4 className="text-xs font-semibold text-slate-400 uppercase flex items-center gap-1"><Tag className="w-3 h-3" /> Classificação</h4>
                 <div className="flex flex-wrap gap-2">
                     ...
                     {card.centro_custo && <Badge variant="secondary" className="bg-slate-800 text-slate-300">{card.centro_custo.nome}</Badge>}
                 </div>
            </div>

            {/* Pagamento */}
            <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-800 space-y-2">
                <h4 className="text-xs font-semibold text-slate-400 uppercase flex items-center gap-1"><Wallet className="w-3 h-3" /> Pagamento</h4>
                <div className="grid grid-cols-2 gap-2 text-sm">
                     <div>
                        <span className="text-xs text-slate-500 block">Valor Original</span>
                        <span className="font-bold text-slate-200">{formatCurrency(card.valor_original)}</span>
                    </div>
                    {card.valor_recebido > 0 && (
                         <div>
                            <span className="text-xs text-slate-500 block">Recebido</span>
                    ...
                    <div>
                         ...
                         <span className="text-slate-300">{card.tipo_pagamento?.nome || '-'}</span>
                    </div>
                    <div>
                         <span className="text-xs text-slate-500 block">Condição</span>
                         <span className="text-slate-300">{card.condicao_pagamento?.nome || '-'}</span>
                    </div>
                </div>
            </div>
            ...
            {/* Documentação */}
            <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-800 space-y-2">
                 <h4 className="text-xs font-semibold text-slate-400 uppercase flex items-center gap-1"><FileText className="w-3 h-3" /> Documentação</h4>
                 <div className="grid grid-cols-2 gap-2 text-sm">
                     <div>
                        <span className="text-xs text-slate-500 block">Tipo Doc</span>
                        <span className="text-slate-200">{card.tipo_documento?.nome || '-'}</span>
                    </div>
                    <div>
                        <span className="text-xs text-slate-500 block">Origem</span>
                        <span className="text-slate-200">{card.origem?.nome || '-'}</span>
                    </div>
                 </div>
            </div>
        </div>
            ...
            {showRecoverForm ? (
                <div className="p-4 bg-emerald-900/10 border border-emerald-900/30 rounded-lg space-y-3">
                    ...
                    <div className="flex gap-2 pt-2">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => setShowRecoverForm(false)} className="text-slate-400 w-full h-8">Cancelar</Button>
                    </div>
                </div>
            ) : (
                <div className="space-y-3">
                    <div className="space-y-1">
                        <Label htmlFor="observacoes" className="text-xs text-slate-400">Observações de Cobrança</Label>
                        <Textarea 
                        id="observacoes"
        ...
        <DialogFooter className="mt-2 pt-4 border-t border-slate-800 flex justify-between items-center w-full">
