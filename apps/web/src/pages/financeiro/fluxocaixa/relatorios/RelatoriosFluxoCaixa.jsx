...
const RelatoriosFluxoCaixa = () => {
  ...
  if (error) {
    return (
        <div className="flex flex-col items-center justify-center h-96 p-6 bg-white rounded-lg shadow-sm border">
             <WifiOff className="h-10 w-10 text-slate-400 mb-4" />
             <h3 className="text-lg font-semibold text-slate-800">Erro de Carregamento</h3>
             <p className="text-slate-500 mb-4 text-center max-w-xs">{error.message}</p>
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10 p-6">
      ...
      <Card className="bg-slate-50 border-slate-200">
        <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium uppercase text-slate-500 flex items-center gap-2">
                <Filter className="w-4 h-4" /> Filtros do Relatório
            </CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-1">
                <label className="text-xs font-bold text-slate-600">Fonte de Dados</label>
            ...
            <div className="space-y-1">
                ...
                <Input type="date" className="bg-white h-9" value={filters.dataInicio} onChange={e => setFilters({...filters, dataInicio: e.target.value})} />
            </div>
            <div className="space-y-1">
                <label className="text-xs font-bold text-slate-600">Data Fim</label>
                <Input type="date" className="bg-white h-9" value={filters.dataFim} onChange={e => setFilters({...filters, dataFim: e.target.value})} />
            </div>
            <div className="space-y-1">
                <label className="text-xs font-bold text-slate-600">Categoria (Busca)</label>
                <Input 
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-12 border rounded-lg bg-white">
            ...
            <p className="text-sm text-slate-500">Atualizando relatório...</p>
        </div>
      ) : (
        <>
            {/* KPIs Summary */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="border-l-4 border-l-slate-400">
                    <CardContent className="p-4">
                        ...
                        <h3 className="text-xl font-mono font-bold text-slate-700">R$ {totals.prev.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                    </CardContent>
                </Card>
                <Card className="border-l-4 border-l-blue-500">
                    <CardContent className="p-4">
                        <p className="text-xs text-slate-500 uppercase font-bold">Total Realizado</p>
                ...
                <Card className={`border-l-4 ${totals.diff >= 0 ? 'border-l-emerald-500' : 'border-l-red-500'}`}>
                    <CardContent className="p-4">
                        <p className="text-xs text-slate-500 uppercase font-bold">Diferença (Real - Prev)</p>
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between py-4">
                    <CardTitle className="text-base flex items-center gap-2">
                        <Eye className="w-4 h-4 text-slate-500" /> Dados
                    </CardTitle>
                    ...
                    <div className="rounded-md border overflow-x-auto max-h-[600px]">
                        <Table>
                            ...
                            <TableBody>
                                {data.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={selectedColumns.length} className="text-center py-8 text-slate-400">
                                    ...
                                    data.map((item, idx) => {
                                        ...
                                        return (
                                            <TableRow key={item.id || idx} className="hover:bg-slate-50">
                                                ...
                                                {selectedColumns.includes('descricao') && <TableCell className="text-xs text-slate-600 truncate max-w-[200px]">{item.descricao}</TableCell>}
                                                {selectedColumns.includes('dreGroup') && <TableCell className="text-center"><span className={`px-2 py-0.5 rounded text-[10px] font-bold ${item.dreGroup === 'A' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>{item.dreGroup}</span></TableCell>}
                                                {selectedColumns.includes('conta') && <TableCell className="text-xs text-slate-500">{item.conta || '-'}</TableCell>}
                                                {selectedColumns.includes('valor_previsto') && <TableCell className="text-right font-mono text-xs text-slate-600">{(item.valor_previsto || 0).toLocaleString('pt-BR', {minimumFractionDigits: 2})}</TableCell>}
