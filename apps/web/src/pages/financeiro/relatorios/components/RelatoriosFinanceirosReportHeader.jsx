...
const RelatoriosFinanceirosReportHeader = ({ title, dateRange, activeFiltersCount, onClearFilters }) => {
  return (
    <div className="bg-background rounded-xl p-6 shadow-xl border border-border relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
        ...
        <div className="flex items-center gap-6 z-10">
            <div className="bg-background p-3 rounded-lg shadow-sm w-32 h-16 flex items-center justify-center shrink-0">
            ...
            <div className="space-y-1">
                ...
                <p className="text-muted-foreground text-sm">ENGAGEMENT Consultoria Empresarial</p>
            </div>
        </div>

        <div className="flex flex-col items-start md:items-end gap-3 z-10 w-full md:w-auto">
            <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/50 px-4 py-2 rounded-full border border-border w-full md:w-auto">
                <CalendarDays className="h-4 w-4 text-orange-500" />
                <span>{dateRange || 'Período Completo (Sem filtro de data)'}</span>
            </div>

            <div className="flex items-center gap-3 w-full md:w-auto">
                <Badge variant="outline" className="bg-muted/80 text-muted-foreground border-border py-1.5 px-3">
