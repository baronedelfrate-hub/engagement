      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="shadow-sm border-slate-200">
            ...
            <CardContent className="p-5 grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
              <div><p className="text-xs text-slate-500 mb-1">Razão Social</p><p className="text-sm font-medium text-slate-900">{nota.clientes?.nome || '-'}</p></div>
              <div><p className="text-xs text-slate-500 mb-1">CNPJ/CPF</p><p className="text-sm text-slate-900">{nota.clientes?.cnpj_cpf || '-'}</p></div>
              <div className="md:col-span-2"><p className="text-xs text-slate-500 mb-1">Endereço</p><p className="text-sm text-slate-700">{nota.clientes?.endereco || '-'}, {nota.clientes?.bairro} - {nota.clientes?.cidade}/{nota.clientes?.estado} - CEP: {nota.clientes?.cep}</p></div>
              
              <div className="col-span-full pt-4 border-t border-slate-100 grid grid-cols-2 gap-4">
                <div><p className="text-xs text-slate-500 mb-1">Natureza da Operação</p><p className="text-sm">{nota.natureza_operacao}</p></div>
                <div><p className="text-xs text-slate-500 mb-1">Pedido Vinculado</p><p className="text-sm font-medium">{nota.pedidos_venda?.numero || 'Não vinculado'}</p></div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm border-slate-200">
            <CardHeader className="py-4 border-b border-slate-100 bg-slate-50/50">
              <CardTitle className="text-base font-semibold text-slate-800">Itens da NF-e</CardTitle>
            </CardHeader>
            <CardContent className="p-0 overflow-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="p-3 font-medium text-slate-600">Produto</th>
                    <th className="p-3 font-medium text-slate-600">NCM</th>
                    <th className="p-3 font-medium text-slate-600 text-right">Qtd</th>
                    <th className="p-3 font-medium text-slate-600 text-right">V. Unit</th>
                    <th className="p-3 font-medium text-slate-600 text-right">Total</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {nota.itens?.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="p-3 text-slate-500">{item.ncm}</td>
                      <td className="p-3 text-right">{item.quantidade}</td>
                      <td className="p-3 text-right">{formatCurrency(item.valor_unitario)}</td>
                      <td className="p-3 text-right font-medium text-slate-800">{formatCurrency(item.valor_total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-8">
                <div className="text-right"><p className="text-xs text-slate-500">Subtotal</p><p className="font-medium text-slate-800">{formatCurrency(nota.subtotal)}</p></div>
                <div className="text-right"><p className="text-xs text-slate-500">Descontos</p><p className="font-medium text-red-600">{formatCurrency(nota.total_descontos)}</p></div>
                <div className="text-right"><p className="text-xs text-slate-500">Impostos</p><p className="font-medium text-slate-800">{formatCurrency(nota.total_impostos)}</p></div>
                <div className="text-right"><p className="text-xs text-slate-500">Valor Total</p><p className="text-lg font-bold text-emerald-600">{formatCurrency(nota.valor_total)}</p></div>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="p-4 space-y-2">
              {nota.protocolo ? (
                <>
                  ...
                  <div className="mt-4 bg-slate-50 p-3 rounded text-xs text-slate-600 break-all">
                    <strong>Protocolo:</strong> {nota.protocolo} <br/>
                    <strong>Autorização:</strong> {nota.autorizacao}
                  </div>
                </>
              ) : (
                <p className="text-sm text-slate-500 text-center py-2">Arquivos disponíveis após emissão.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm border-slate-200">
            <CardHeader className="py-4 border-b border-slate-100 bg-slate-50/50">
              <CardTitle className="text-base font-semibold text-slate-800 flex items-center gap-2">
                <History className="w-4 h-4 text-slate-600" /> Histórico de Eventos
              </CardTitle>
            </CardHeader>
                  ...
                  {nota.historico?.map((h) => (
                    <div key={h.id} className="relative">
                      ...
                      <div className="flex flex-col">
                        ...
                        {h.detalhes && <span className="text-xs text-slate-600 mt-0.5">{h.detalhes}</span>}
