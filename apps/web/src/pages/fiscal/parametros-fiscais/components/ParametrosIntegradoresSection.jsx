...
export default function ParametrosIntegradoresSection({ data, onChange, onSave, onTestConnection, loading }) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'Conectado': return 'bg-emerald-100 text-emerald-800 border-emerald-200';
      ...
      <Card className="shadow-sm border-slate-200">
        ...
        <CardContent className="p-6 flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="space-y-1 md:border-l md:border-slate-200 md:pl-6">
            ...
            <p className="font-medium text-slate-900">{data.ultima_sincronizacao ? new Date(data.ultima_sincronizacao).toLocaleString() : 'Nunca realizada'}</p>
          </div>
          <div className="space-y-1 md:border-l md:border-slate-200 md:pl-6">
            <p className="text-sm text-slate-500">Ambiente Selecionado</p>
            <p className="font-medium text-slate-900">{data.ambiente || 'Produção'}</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="shadow-sm border-slate-200">
        <CardHeader className="bg-slate-50/50 border-b border-slate-100">
          <CardTitle className="text-base flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-slate-600" /> API e Credenciais
          </CardTitle>
        </CardHeader>
      ...
      <Card className="shadow-sm border-slate-200">
        ...
        <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <div className="border-2 border-dashed border-slate-200 rounded-lg p-6 flex flex-col items-center justify-center text-slate-500 bg-slate-50/50 hover:bg-slate-100 transition-colors cursor-pointer">
